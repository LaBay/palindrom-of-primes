...
* начинаться с цифры 9 и, следовательно, заканчиваться на 9.
* Значит, необходимо использовать только те простые числа, которые начинаются и 
* заканчиваются 1, 3, 9 (т.е. делителями 9).

* II
* Очевидно, что перемножение пятизначных цифр требует значительных занимает 
* значительное время и производительность компьютера.
* При чём следует перемножать 1 с 9, а 3 с 3.
* Поэтому формируется 9 массивов для перемножение цифр:
* 	1..1 с 9..9
* 	1..3 с 9..3
* 	1..9 с 9..1
* 	3..1 с 3..9
